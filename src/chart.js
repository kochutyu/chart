...
import {boundaries, circle, computeXRatio, computeYRatio, css, isOver, line, toCoords, toDate,} from './utils'
...
export function chart(root, data) {
    ...
    let raf
    let prevMax
    canvas.width = DPI_CHART_WIDTH
    canvas.height = DPI_CHART_HEIGHT
    css(canvas, {
    ...
    function clear() {
        ctx.clearRect(0, 0, DPI_CHART_WIDTH, DPI_CHART_HEIGHT)
    }
    ...
    function paint() {
        ...
        yData
            .map(toCoords(xRatio, yRatio, DPI_CHART_HEIGHT, PADDING, yMin))
            .forEach((coords, idx) => {
                const color = data.colors[yData[idx][0]]
                line(ctx, coords, {color})

                for (const [x, y] of coords) {
                    if (isOver(proxy.mouse, x, coords.length, DPI_CHART_WIDTH)) {
    ...
    function xAxis(xData, yData, xRatio) {
        ...
        for (let i = 1; i < xData.length; i++) {
            ...
            if ((i - 1) % step === 0) {
                ...
                ctx.fillText(text.toString(), x, DPI_CHART_HEIGHT - 10)
            }

            if (isOver(proxy.mouse, x, xData.length, DPI_CHART_WIDTH)) {
                ctx.save()
                ctx.moveTo(x, PADDING / 2)
                ctx.lineTo(x, DPI_CHART_HEIGHT - PADDING)
                ctx.restore()
    ...
    function yAxis(yMin, yMax) {
        ...
        for (let i = 1; i <= ROWS_COUNT; i++) {
            ...
            ctx.lineTo(DPI_CHART_WIDTH, y + PADDING)
        }
        ctx.stroke()
        ctx.closePath()
    }
