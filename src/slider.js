...
import {DPI_SLIDER_HEIGHT, SLIDER_HEIGHT} from "./config";
...
export function sliderChart(root, data, DPI_WIDTH) {
    ...
    canvas.height = DPI_SLIDER_HEIGHT
    css(canvas, {
        width: WIDTH + 'px',
    ...
    const yRatio = computeYRatio(DPI_SLIDER_HEIGHT, yMax, yMin)
    const xRatio = computeXRatio(DPI_WIDTH, data.columns[0].length)

    const yData = data.columns.filter((col) => data.types[col[0]] === 'line')

    yData
        .map(toCoords(xRatio, yRatio, DPI_SLIDER_HEIGHT, 0, yMin))
        .forEach((coords, idx) => {
